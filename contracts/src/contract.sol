...
/**
 * Access control and judge for contracts
 */
contract Authority {
    ...
    function createBuyContract(address company, string buyerPubKey, string dataH, string idH) payable returns (address) {
        ...
        // Check black-list of seller
        assert(!isSellerBlocked(buyInfo.seller()));
        // Check black-list of buyer
        assert(!isBuyerBlocked(buyInfo.buyer()));
        // Check pub-key unique
        bytes32 sign = buyInfo.getPubKeySign();
        assert(!isPubKeySignUsed(sign, buyInfo.buyer()));
        // Remember pub key
        markPubKeySignAsUsed(sign, buyInfo.buyer());
        return buyInfo;
...
contract BuyInfo {
    ...
    event DataUploaded();         // Notify that data uploaded by seller
